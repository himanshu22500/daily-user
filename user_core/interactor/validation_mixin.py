...
from user_core.exceptions.expections import InvalidManagerId, ManagerDoesNotExists, DeactivatedManager, \
    InvalidMobileNumber, InvalidUserIds, MobileNumberAlreadyExists, PanNumberAlreadyExists, InvalidPanNumber
...
class ValidationMixin:
    ...
    @staticmethod
    def validate_and_adjust_pan_number(
            user_storage:UserStorageInterface,
            pan_number:str
    ):
        is_pan_number_valid = re.match(r'[A-Za-z]{5}[0-9]{4}[A-Za-z]{1}', pan_number) is not None
        if not is_pan_number_valid:
            raise InvalidPanNumber(pan_number=pan_number)
    ...
    def validate_and_adjust_mobile_number(user_storage:UserStorageInterface, mobile_number:str):
        ...
        is_mobile_number_valid = re.match(r'^[0-9]{10}$', mobile_number) is not None
